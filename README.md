...
## 2026-09-15 の追加修正(中村さん指示・文字サイズと太字)
1. 肩書き「再生の道 府中市ボランティア／交通コンサルタント」を拡大(PC 13→15px / SP 11→13px)
   - 拡大分は字間(.16em→.04em)と区切り線幅(24→20px)を詰めて**1行維持**
   - SPは横線の区切りを「／」の文字に置換し、`word-break:keep-all` で語中改行を防止
     (横線のままだと端末幅により行末に線だけが取り残されるため)
2. フリガナ「むらこし じゅんた」を拡大(ヒーロー 12→15px / PROFILE欄 11→13px。字間は微調整)
3. セクション見出し(MESSAGE / ACTIVITY / PROFESSION / YOUTUBE / WEEKLY REPORT / PROFILE /
   DONATION / FOLLOW)を **12→15px・font-weight 500→700(太字)** に統一(SPは13px)
4. CSSのキャッシュバスターを `?v=20260915b` に更新(公開後すぐ反映させるため)

### 検証(追加修正分)
- 画面幅 320/360/390/768/1025/1150/1400/1920 で横オーバーフロー **0**
- 8つのセクション見出しが全幅で 15px(SP 13px)/太字700・**すべて1行**であることを機械確認
- 肩書きは 390px以上で1行、360px以下は「／」位置で2行に折返し(語中では折れない)
